test_that("with_text works", {
    fix_issue <- with_text(tested_issues, "fix")
    ...
    typo_issue <- with_text(tested_issues, "typo")
    expect_issues(typo_issue)
    expect_identical(nrow(typo_issue), 0L)

    fix_issue2 <- with_text(tested_issues, "fix", ignore.case = TRUE)
    ...
    awful_issue <- with_text(tested_issues, "awful", in_body = FALSE)
    expect_issues(awful_issue)
    expect_identical(nrow(awful_issue), 1L)
})

test_that("with_labels works", {
    bug_issues <- with_labels(tested_issues, "bug")
...
test_that("with_comments works", {
    commented_issues <- with_comments(tested_issues)
    ...
    not_commented_issues <- with_comments(tested_issues, negate = TRUE)
...
test_that("get_nbr_comments works", {
    expect_identical(
        object = get_nbr_comments(tested_issues),
...
test_that("author_last_comment works", {
    expect_identical(
        object = author_last_comment(tested_issues),
        expected = c("", "", "", "palatej", "", "palatej")
    )
    expect_identical(
        object = author_last_comment(tested_issues[6L, ]),
        expected = "palatej"
    )
    expect_message(expect_identical(
        object = author_last_comment(tested_issues[1, ]),
        expected = ""
    ))
})

test_that("extract_nth works", {
    expect_message(first_issue <- extract_nth(tested_issues, 1L))
