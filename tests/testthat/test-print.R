...
test_that("print works for IssuesTB object", {
    expect_output(
        object = print(tested_issues[1:2, ]),
...
test_that("print works for summary.IssueTB object", {
    expect_output(
        object = print(summary(tested_issues[1, ])),
        regexp = "Fix config import in ProvidersTopComponent"
    )
    expect_output(
        object = print(summary(tested_issues[1, ])),
...
test_that("print works for summary.IssuesTB object", {
    expect_output(
        object = print(summary(tested_issues[1:2, ])),
        regexp = "There are 2 issues."
    )
    expect_output(
        object = print(summary(tested_issues[1:2, ])),
...
test_that("print works for summary.IssuesTB object with labels", {
    expect_output(
        object = print(summary(tested_issues[1:2, ], with_labels = TRUE)),
        regexp = "bug"
    )
})
...
test_that("print works for summary.IssuesTB object without labels", {
    expect_error(
        expect_output(
            object = print(summary(tested_issues[1:2, ], with_labels = FALSE)),
            regexp = "bug"
        )
    )
})
